module.exports = (function() {
    ...
    if (Object.hasOwnProperty.call(cache, value)) {
      ...
      var font = /sb-comment-label/.test(this.cls)
        ...
        : /sb-literal/.test(this.cls)
          ? "normal 12px " + defaultFontFamily
          : "bold 12px " + defaultFontFamily
  ...
  Icon.icons = {
    greenFlag: { width: 10, height: 5, dy: -8 }, ///working on this
  ...
  Input.prototype.draw = function(parent) {
    if (this.hasLabel) {
      ...
      var w = this.isInset ? 25 : this.isColor ? 25 : null
    }
    if (this.hasArrow) w += 15
    this.width = w

    var h = (this.height = this.isRound || this.isColor ? 22 : 22)
    ...
    if (this.hasLabel) {
      ...
      result.appendChild(SVG.move(x, 5, label))
    }
    if (this.hasArrow) {
